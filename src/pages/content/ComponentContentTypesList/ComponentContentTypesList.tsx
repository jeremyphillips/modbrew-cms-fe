...
import type { ComponentSchemaApiPayload } from '~api/componentSchema'

const ComponentContentTypesList = () => {
  const [components, setComponents] = useState<ComponentSchemaApiPayload>([])
  ...
  useEffect(() => {
    const getComponents = async () => {
      try {
        ...
        setComponents(data as ComponentSchemaApiPayload)
      } catch (err) {
        ...
        <ul>
          {components.map(component => (
            <li key={component._id} className="mb-4">
              ...
              <Link className="text-blue-500 hover:underline" to={`/component-content/create/${component._id}`}>
